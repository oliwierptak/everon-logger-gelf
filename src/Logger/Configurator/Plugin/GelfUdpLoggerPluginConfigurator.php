...
class GelfUdpLoggerPluginConfigurator extends \Everon\Logger\Plugin\Gelf\AbstractGelfPluginConfigurator implements \Everon\Logger\Contract\Configurator\PluginConfiguratorInterface
{
    ...
    protected const METADATA = [
        'chunkSize' => ['type' => 'int', 'default' => \Gelf\Transport\UdpTransport::CHUNK_SIZE_WAN],
        'host' => ['type' => 'string', 'default' => \Gelf\Transport\UdpTransport::DEFAULT_HOST],
        'ignoreTransportErrors' => ['type' => 'bool', 'default' => true],
        'logLevel' => ['type' => 'string', 'default' => 'debug'],
        'pluginClass' => ['type' => 'string', 'default' => \Everon\Logger\Plugin\GelfUdp\GelfUdpLoggerPlugin::class],
        'pluginFactoryClass' => ['type' => 'string', 'default' => null],
        'port' => ['type' => 'int', 'default' => \Gelf\Transport\UdpTransport::DEFAULT_PORT],
        'shouldBubble' => ['type' => 'bool', 'default' => true],
    ];

    /** defaults to CHUNK_SIZE_WAN, 0 disables chunks completely */
    protected ?int $chunkSize = \Gelf\Transport\UdpTransport::CHUNK_SIZE_WAN;

    /** when NULL or empty default-host is used */
    protected ?string $host = \Gelf\Transport\UdpTransport::DEFAULT_HOST;
    ...
    protected ?string $pluginClass = \Everon\Logger\Plugin\GelfUdp\GelfUdpLoggerPlugin::class;
    ...
    protected ?int $port = \Gelf\Transport\UdpTransport::DEFAULT_PORT;
    ...
    {
        $this->chunkSize = $chunkSize; $this->updateMap['chunkSize'] = true; return $this;
    }
    ...
    {
        if (static::METADATA['chunkSize']['type'] === 'popo' && $this->chunkSize === null) {
            $popo = static::METADATA['chunkSize']['default'];
            $this->chunkSize = new $popo;
        }

        if ($this->chunkSize === null) {
            throw new UnexpectedValueException('Required value of "chunkSize" has not been set');
        }
        return $this->chunkSize;
    }
    ...
    {
        return $this->chunkSize !== null || ($this->chunkSize !== null && array_key_exists('chunkSize', $this->updateMap));
    }
    ...
    public function requireAll(): self
    {
        ...
        try {
            $this->requireChunkSize();
        }
        catch (\Throwable $throwable) {
            $errors['chunkSize'] = $throwable->getMessage();
        }
        try {
            $this->requireHost();
        }
